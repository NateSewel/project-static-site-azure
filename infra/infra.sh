...
echo "[2/11] Creating resource group: ${RG_NAME}"
az group create --name "${RG_NAME}" --location "${AZ_LOCATION}" --output none
...
az network public-ip create --resource-group "${RG_NAME}" --name "${PUBLIC_IP_NAME}" \
